...
// Cache parsed lines for each diff to avoid recalculation
  ...
  <div v-else class="diff-viewer bg-background select-text">
    <div v-for="(diff, i) in parsedDiffs" :key="i" class="diff-file border-b border-border last:border-b-0">
      ...
      <div class="diff-content font-mono text-[11px] bg-card overflow-hidden">
        ...
        <div v-if="diff.lines.truncated" class="p-4 text-center bg-muted/20 text-muted-foreground text-[10px] italic border-t border-border/10">
          ... Showing only first {{ MAX_LINES_PER_FILE }} of {{ diff.lines.count }} lines. File too large to display fully.
        </div>
      </div>
    </div>
  </div>
